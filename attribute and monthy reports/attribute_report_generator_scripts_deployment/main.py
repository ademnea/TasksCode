...
def get_date_range_and_attribute(args):
    ...
    parser.add_argument('--attribute', type=str, help='Attribute to analyze (carbondioxide, humidity, temperature, weight)')
    ...
    file_paths = {
        ...
        'temperature': args.temp_file or os.getenv('BEEHIVE_TEMPERATURE_CSV', '/home/ltgwgeorge/Desktop/IoT-RA/forAnalytics/beehive_reporting/csv_files/hive_temperatures_hive1.csv'),
        'weight': args.weight_file or os.getenv('BEEHIVE_WEIGHT_CSV', '/home/ltgwgeorge/Desktop/IoT-RA/forAnalytics/beehive_reporting/csv_files/hive_weights_hive1.csv')
    }
    ...
    if args.attribute:
        ...
        attribute = validate_attribute(os.getenv('BEEHIVE_ATTRIBUTE'))
    ...
    if args.start_date and args.end_date:
        ...
        return generate_month_range(start_year, start_month, end_year, end_month), file_paths, hive_id, output_dir, download_dir, attribute
    ...
    if args.year and args.month:
        ...
        return [(year, month)], file_paths, hive_id, output_dir, download_dir, attribute
    ...
    if sys.stdin.isatty():  # Interactive environment
        try:
            # Prompt for attribute if not provided
            if not attribute:
                valid_attributes = ['carbondioxide', 'humidity', 'temperature', 'weight']
                attribute_input = input(f"Enter attribute to analyze ({', '.join(valid_attributes)}) [default: {default_attribute}]: ").strip() or default_attribute
                attribute = validate_attribute(attribute_input)
        ...
        return [(current_year, current_month)], file_paths, hive_id, output_dir, download_dir, attribute
...
if __name__ == "__main__":
    # Get date range, file paths, and attribute
    try:
        year_month_list, file_paths, hive_id, output_dir, download_dir, attribute = get_date_range_and_attribute(sys.argv)
    except ValueError as e:
        logging.error(str(e))
        sys.exit(1)

    # Load and clean data based on attribute
    df_dict = {}
    try:
        if attribute == 'carbondioxide':
    ...
    # Range of months (weight):
    # python main.py --start_date 03/2025 --end_date 06/2025 --attribute weight \
    #   --weight_file /path/to/hive_weights_hive1.csv \
    #   --hive_id hive1 \
    #   --output_dir /path/to/monthly_reports \
    #   --download_dir /path/to/downloads
    # Single month (carbondioxide):
    # python main.py --year 2025 --month 3 --attribute carbondioxide \
    #   --co2_file /path/to/hive_carbondioxide_hive1.csv \
    #   --hive_id hive1 \
    #   --output_dir /path/to/monthly_reports
