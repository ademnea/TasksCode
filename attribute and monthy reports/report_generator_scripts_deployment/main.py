...
def main(df_co2, df_weight, df_temperature, df_humidity, year_month_list, hive_id, output_dir, download_dir=None):
    logging.info(f"Starting analysis for {len(year_month_list)} month(s)")
    
    # Initialize collections for all results and plots
    all_results = {}
    all_plots = {}
    ...
    for year, month in year_month_list:
        logging.info(f"Analyzing year={year}, month={month}")
        ...
        # Collect plot filenames with full paths, only if the section result has valid data
        all_plots[month_key] = {}
        if not co2_results.get("Error"):
            all_plots[month_key]["CO2 Analysis"] = [
        ...
        if not weight_results.get("Error"):
            all_plots[month_key]["Weight Analysis"] = [os.path.join(report_folder, "weight_monthly_trend.png")]
        if not temperature_results.get("Error"):
            all_plots[month_key]["Temperature Analysis"] = [os.path.join(report_folder, "temperature_trend.png")]
        if not humidity_results.get("Error"):
            all_plots[month_key]["Humidity Analysis"] = [os.path.join(report_folder, "humidity_trend.png")]
        if not correlation_results.get("Error"):
            all_plots[month_key]["Correlation Analysis"] = [
    ...
    # Generate a single PDF for all months
    report_folder = os.path.join(output_dir, f"Range_{start_year}_{start_month_name}_to_{end_year}_{end_month_name}")
    os.makedirs(report_folder, exist_ok=True)
    pdf_filename = generate_pdf(all_results, all_plots, start_year, start_month, end_year, end_month, hive_id, report_folder)
